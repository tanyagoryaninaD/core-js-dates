...
function getNextFriday(date) {
  ...
  const daysToAdd = daysToFriday === 0 ? 7 : daysToFriday;
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  const msBetween = new Date(dateEnd) - new Date(dateStart);
  return Math.floor(msBetween / 86400000 + 1);
}
...
function getNextFridayThe13th(date) {
  let findDate = date;
  findDate = new Date(findDate.setUTCMonth(findDate.getUTCMonth() + 1));
  findDate = new Date(findDate.setUTCDate(13));

  while (findDate.getUTCDay() !== 5) {
    findDate = new Date(findDate.setUTCMonth(findDate.getUTCMonth() + 1));
    findDate = new Date(findDate.setUTCDate(13));
  }

  return new Date(findDate.getUTCFullYear(), findDate.getUTCMonth(), 13);
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  const currentDate = new Date(Date.UTC(start[0], start[1] - 1, start[2]));
  const endDate = new Date(Date.UTC(end[0], end[1] - 1, end[2]));
  const workSchedule = [];

  while (currentDate <= endDate) {
    for (let i = 0; i < countWorkDays && currentDate <= endDate; i += 1) {
      workSchedule.push(createFormatDate(currentDate));
...
function isLeapYear(date) {
  ...
  const checkFour = year % 4 === 0;
  const checkHundred = year % 100 === 0;
  const checkFourHundred = year % 400 === 0;

  return (checkFour && !checkHundred) || checkFourHundred;
}
